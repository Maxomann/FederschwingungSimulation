...
//Feder::Feder( float graviationskonstante/*m/s^2*/, float masse/*kg*/, float federkonstante/*N/m*/ )
//	:kraftNachUnten( masse*graviationskonstante*-1 ),
//	masse( masse ),
...
Feder::Feder( sf::Vector2f drawingOffset, float fallbeschleunigung/*m/s^2*/, float masse/*kg*/, float federkonstante/*N/m*/ )
	:fallbeschleunigung( fallbeschleunigung*-1 ),
	federlängeInRuhe( (fallbeschleunigung*masse)/federkonstante ),
...
void Feder::frame( float deltaT )
{
	...
	wirkendeBeschleunigung *= deltaT;

	geschwindigkeit += wirkendeBeschleunigung;
...
void Feder::hebAn( const float weg )
{
	if( weg > 0 )
		vorzeichen = false;
	else if( weg < 0 )
...
vector<float> Feder::getData() const
{
